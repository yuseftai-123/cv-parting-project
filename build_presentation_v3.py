...
COLOR_LIGHT_GRAY = RGBColor(215, 222, 232)
...
def update_simple_text(shape, text, font_size=Pt(13), bold=False, color=COLOR_DARK, font_name=FONT_PRIMARY):
...
def update_title_accent(shape, line1, line2, line1_size=Pt(30), line2_size=Pt(30), font_name=FONT_PRIMARY):
    if not shape.has_text_frame:
        return
    tf = shape.text_frame
    format_text_frame(tf)
...
for shape in s1.shapes:
    if shape.shape_id == 15:
        update_simple_text(shape, "KINOVA TECH", font_size=Pt(18), bold=True, color=COLOR_ORANGE)
    elif shape.shape_id == 16:
        update_title_accent(shape, "CV Parser Engine", "Rapport de Stage", line1_size=Pt(42), line2_size=Pt(42))
    elif shape.shape_id == 17:
        ...
        update_simple_text(shape, "Présenté par", font_size=Pt(13), color=COLOR_GRAY)
    elif shape.shape_id == 19:
    ...
    elif shape.shape_id == 20:
        update_simple_text(shape, "EST Meknès — UMI · Génie Informatique · 2025/2026", font_size=Pt(13), bold=True, color=COLOR_WHITE)
...
for shape in s2.shapes:
    if shape.shape_id == 47:
        update_simple_text(shape, "02", font_size=Pt(20), bold=True, color=COLOR_WHITE)
        ...
        update_title_accent(shape, "LE TRI MANUEL DE CVS", "FREINE LE RECRUTEMENT", line1_size=Pt(30), line2_size=Pt(30))
    elif shape.shape_id == 49:
        update_simple_text(shape, "Les cabinets de recrutement et directions RH reçoivent quotidiennement des milliers de candidatures dans des formats hétérogènes (PDF, DOCX, scans), ce qui ralentit le processus et augmente les risques.", font_size=Pt(13), color=COLOR_GRAY)
    
    # Corrected Card Mappings
        ...
        update_simple_text(shape, "TEMPS PERDU", font_size=Pt(13), bold=True, color=COLOR_DARK)
        ...
        update_simple_text(shape, "DONNÉES HÉTÉROGÈNES", font_size=Pt(13), bold=True, color=COLOR_DARK)
        ...
        update_simple_text(shape, "RISQUE DE CONFORMITÉ", font_size=Pt(13), bold=True, color=COLOR_DARK)
        ...
        update_simple_text(shape, "MANQUE DE VISIBILITÉ", font_size=Pt(13), bold=True, color=COLOR_DARK)
...
for shape in s3.shapes:
    if shape.shape_id == 32:
        update_simple_text(shape, "03", font_size=Pt(20), bold=True, color=COLOR_WHITE)
        ...
        update_title_accent(shape, "KINOVATECH CV PARSER ENGINE,", "LA SOLUTION QUI FAIT AVANCER LE RECRUTEMENT", line1_size=Pt(26), line2_size=Pt(26))
    elif shape.shape_id == 34:
        update_simple_text(shape, "Un moteur intelligent qui ingère, anonymise, structure et enrichit automatiquement chaque CV — du fichier brut à la fiche candidat exploitable, en conformité CNDP.", font_size=Pt(13), color=COLOR_GRAY)
    elif shape.shape_id == 35:
        update_simple_text(shape, "43 / 43", font_size=Pt(22), bold=True, color=COLOR_WHITE)
        ...
        update_simple_text(shape, "Validation de l'ensemble des cas d'utilisation.", font_size=Pt(11), color=COLOR_LIGHT_GRAY)
    elif shape.shape_id == 38:
        update_simple_text(shape, "< 0.5s", font_size=Pt(22), bold=True, color=COLOR_WHITE)
        ...
        update_simple_text(shape, "Exécution haute performance.", font_size=Pt(11), color=COLOR_LIGHT_GRAY)
    elif shape.shape_id == 41:
        update_simple_text(shape, "3", font_size=Pt(22), bold=True, color=COLOR_WHITE)
        ...
        update_simple_text(shape, "Ingestion, Anonymisation & Extraction.", font_size=Pt(11), color=COLOR_LIGHT_GRAY)
...
for shape in s4.shapes:
    if shape.shape_id == 50:
        update_simple_text(shape, "04", font_size=Pt(20), bold=True, color=COLOR_WHITE)
    elif shape.shape_id == 51:
        update_simple_text(shape, "04 — L'ARCHITECTURE", font_size=Pt(13), bold=True, color=COLOR_ORANGE)
    elif shape.shape_id == 9:
        update_title_accent(shape, "UNE ARCHITECTURE", "PENSÉE POUR LA CONFORMITÉ ET L'ÉCHELLE", line1_size=Pt(28), line2_size=Pt(28))
    elif shape.shape_id == 52:
        update_simple_text(shape, "Le moteur repose sur une stack moderne combinant extraction documentaire, NLP et validation stricte des données, prête à monter en charge.", font_size=Pt(13), color=COLOR_GRAY)
    
    # Orange card 1
    elif shape.shape_id == 55:
        update_simple_text(shape, "EXTRACTION MULTI-FORMAT (SF-01)", font_size=Pt(13), bold=True, color=COLOR_DARK)
    elif shape.shape_id == 54:
        update_simple_text(shape, "Support natif PDF (PyMuPDF) et DOCX (python-docx) avec préservation des blocs de texte.", font_size=Pt(12), color=COLOR_DARK)
    
    # Dark card 2 (White text)
    elif shape.shape_id == 57:
        update_simple_text(shape, "SCHÉMA DE DONNÉES UNIFIÉ (SF-08)", font_size=Pt(13), bold=True, color=COLOR_WHITE)
    elif shape.shape_id == 56:
        update_simple_text(shape, "Validation stricte via Pydantic v2, conforme au schéma JSON officiel (Section 3.3).", font_size=Pt(12), color=COLOR_LIGHT_GRAY)
    
    # Dark card 3 (White text)
    elif shape.shape_id == 59:
        update_simple_text(shape, "NLP & AUTOMATISATION (SF-05)", font_size=Pt(13), bold=True, color=COLOR_WHITE)
    elif shape.shape_id == 58:
        update_simple_text(shape, "spaCy fr_core_news_lg avec 94.2% de précision sur les entités en français.", font_size=Pt(12), color=COLOR_LIGHT_GRAY)
    
    # Orange card 4
    elif shape.shape_id == 61:
        update_simple_text(shape, "CONÇU POUR MONTER EN CHARGE (SF-09)", font_size=Pt(13), bold=True, color=COLOR_DARK)
    elif shape.shape_id == 60:
        update_simple_text(shape, "Traitement batch ZIP jusqu'à 500 CVs ; infrastructure Docker (PostgreSQL, Redis, MinIO).", font_size=Pt(12), color=COLOR_DARK)
        ...
        update_simple_text(shape, "Haute Performance & Conformité Native", font_size=Pt(13), color=COLOR_LIGHT_GRAY)
...
for shape in s5.shapes:
    if shape.shape_id == 44:
        update_simple_text(shape, "05", font_size=Pt(20), bold=True, color=COLOR_WHITE)
    elif shape.shape_id == 45:
        update_simple_text(shape, "05 — LE PIPELINE EN ACTION", font_size=Pt(13), bold=True, color=COLOR_ORANGE)
    elif shape.shape_id == 9:
        update_title_accent(shape, "8 ÉTAPES. 3 COUCHES.", "UN SEUL PIPELINE.", line1_size=Pt(30), line2_size=Pt(30))
    elif shape.shape_id == 46:
        update_simple_text(shape, "Chaque CV traverse un pipeline séquentiel qui transforme un fichier brut en fiche candidat structurée, anonymisée et enrichie.", font_size=Pt(13), color=COLOR_GRAY)
    elif shape.shape_id == 48:
...
for shape in s6.shapes:
    if shape.shape_id == 32:
        update_simple_text(shape, "06", font_size=Pt(20), bold=True, color=COLOR_WHITE)
    elif shape.shape_id == 33:
        update_simple_text(shape, "06 — AVANT VS APRÈS", font_size=Pt(13), bold=True, color=COLOR_ORANGE)
    elif shape.shape_id == 31:
        update_title_accent(shape, "LA DIFFÉRENCE", "EST CLAIRE", line1_size=Pt(32), line2_size=Pt(32))
    elif shape.shape_id == 34:
        update_simple_text(shape, "Du tri manuel au traitement automatisé et sécurisé — ce que le moteur change concrètement.", font_size=Pt(13), color=COLOR_GRAY)
    elif shape.shape_id == 36:
        update_simple_text(shape, "AVANT (Tri Manuel)", font_size=Pt(13), bold=True, color=COLOR_DARK)
    elif shape.shape_id == 37:
        update_simple_text(shape, "APRÈS (CV Parser Engine)", font_size=Pt(13), bold=True, color=COLOR_ORANGE)
    elif shape.shape_id == 38:
    ...
    # Bottom left dark box: White text
    elif shape.shape_id == 35:
        update_simple_text(shape, "Le bon outil ne change pas seulement la façon de travailler — il transforme ce que l'on peut prouver.", font_size=Pt(13), bold=True, color=COLOR_WHITE)
...
for shape in s7.shapes:
    if shape.shape_id == 76:
        update_simple_text(shape, "07", font_size=Pt(20), bold=True, color=COLOR_WHITE)
    elif shape.shape_id == 77:
        ...
        update_simple_text(shape, "Le moteur a été validé par une suite de tests automatisés couvrant chaque couche du pipeline.", font_size=Pt(13), color=COLOR_GRAY)
...
for shape in s8.shapes:
    if shape.shape_id == 39:
        update_simple_text(shape, "08", font_size=Pt(20), bold=True, color=COLOR_WHITE)
    elif shape.shape_id == 40:
        update_simple_text(shape, "08 — CONFORMITÉ LÉGALE", font_size=Pt(13), bold=True, color=COLOR_ORANGE)
    elif shape.shape_id == 9:
        update_title_accent(shape, "PROTÉGER LES DONNÉES.", "RESPECTER LA LOI.", line1_size=Pt(30), line2_size=Pt(30))
    elif shape.shape_id == 41:
        update_simple_text(shape, "Le projet applique concrètement la Loi n°09-08 (CNDP) à chaque étape du traitement.", font_size=Pt(13), color=COLOR_GRAY)
    
    # Top right banner (White text)
    elif shape.shape_id == 54:
        update_simple_text(shape, "Une conformité intégrée nativement, pas ajoutée après coup.", font_size=Pt(13), bold=True, color=COLOR_WHITE)

    # Bottom left dark stat box (White / light text)
    elif shape.shape_id == 42:
        update_simple_text(shape, "Couche 1", font_size=Pt(22), bold=True, color=COLOR_ORANGE)
    elif shape.shape_id == 43:
        update_simple_text(shape, "Anonymisation immédiate", font_size=Pt(11), color=COLOR_LIGHT_GRAY)
    elif shape.shape_id == 44:
        update_simple_text(shape, "MINIMISATION", font_size=Pt(11), bold=True, color=COLOR_WHITE)
    elif shape.shape_id == 45:
        update_simple_text(shape, "100%", font_size=Pt(22), bold=True, color=COLOR_ORANGE)
    elif shape.shape_id == 46:
        update_simple_text(shape, "Gate de sécurité validé", font_size=Pt(11), color=COLOR_LIGHT_GRAY)
    elif shape.shape_id == 47:
        update_simple_text(shape, "SECURITY GATE", font_size=Pt(11), bold=True, color=COLOR_WHITE)
    elif shape.shape_id == 48:
        update_simple_text(shape, "Art. 7", font_size=Pt(22), bold=True, color=COLOR_ORANGE)
    elif shape.shape_id == 49:
        update_simple_text(shape, "Droit à l'oubli & Purge", font_size=Pt(11), color=COLOR_LIGHT_GRAY)
    elif shape.shape_id == 50:
        update_simple_text(shape, "CONFORMITÉ CNDP", font_size=Pt(11), bold=True, color=COLOR_WHITE)
    elif shape.shape_id == 51:
        update_simple_text(shape, "0 PII", font_size=Pt(22), bold=True, color=COLOR_ORANGE)
    elif shape.shape_id == 52:
        update_simple_text(shape, "Aucune fuite dans les logs", font_size=Pt(11), color=COLOR_LIGHT_GRAY)
    elif shape.shape_id == 53:
        update_simple_text(shape, "TRAÇABILITÉ", font_size=Pt(11), bold=True, color=COLOR_WHITE)
...
for shape in s9.shapes:
    if shape.shape_id == 55:
        update_simple_text(shape, "09", font_size=Pt(20), bold=True, color=COLOR_WHITE)
    elif shape.shape_id == 56:
        update_simple_text(shape, "09 — ENCADREMENT & PARTIES PRENANTES", font_size=Pt(13), bold=True, color=COLOR_ORANGE)
    elif shape.shape_id == 54:
        update_title_accent(shape, "UN STAGE ENCADRÉ.", "UN PROJET PORTÉ.", line1_size=Pt(30), line2_size=Pt(30))
    elif shape.shape_id == 57:
        update_simple_text(shape, "Ce projet a été mené en autonomie, avec un encadrement resserré côté académique et entreprise.", font_size=Pt(13), color=COLOR_GRAY)
    
    # Bottom left orange box (White text)
    elif shape.shape_id == 58:
        update_simple_text(shape, "Un encadrement.\nUne confiance renouvelée chaque jour.", font_size=Pt(14), bold=True, color=COLOR_WHITE)
...
for shape in s10.shapes:
    if shape.shape_id == 41:
        update_simple_text(shape, "10", font_size=Pt(20), bold=True, color=COLOR_WHITE)
    elif shape.shape_id == 42:
        update_simple_text(shape, "10 — MERCI", font_size=Pt(13), bold=True, color=COLOR_ORANGE)
    elif shape.shape_id == 40:
        update_title_accent(shape, "MERCI !", "CONSTRUISONS LA SUITE.", line1_size=Pt(30), line2_size=Pt(30))
    elif shape.shape_id == 43:
        update_simple_text(shape, "La meilleure façon de prévoir l'avenir, c'est de le construire — une fonctionnalité à la fois.", font_size=Pt(13), color=COLOR_GRAY)
    elif shape.shape_id == 45:
        update_simple_text(shape, "Perspectives de déploiement : file asynchrone Celery + Redis, orchestration Kubernetes (AWS EKS), module OCR Tesseract pour les CVs scannés.", font_size=Pt(13), color=COLOR_GRAY)
    elif shape.shape_id == 44:
        update_simple_text(shape, "Merci à Kinova Tech et à l'EST Meknès (UMI) pour cette expérience à distance.", font_size=Pt(13), bold=True, color=COLOR_DARK)
    elif shape.shape_id == 37:
...
print(f"Successfully generated updated layout presentation v3 at: {output_path}")
